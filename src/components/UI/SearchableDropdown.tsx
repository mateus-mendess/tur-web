...
export function SearchableDropdown({
  ...
  return (
    <div className="relative">
      ...
      {/* Popover */}
      <div
        className={`absolute left-0 top-full bg-white border border-black/20 shadow-2xl z-30 flex flex-col rounded-none transition-all duration-200 ease-out transform origin-top-left ${
          isOpen
      ...
      >
        {/* Input de Busca */}
        <div className="flex items-center w-full border border-black/30 rounded-none focus-within:border-black">
          <input
            type="text"
            placeholder={placeholder}
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className={`flex-1 font-inter text-xs bg-transparent border-none rounded-none outline-none focus:ring-0 ${styles.input}`}
          />
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="12"
            ...
            strokeLinecap="round"
            strokeLinejoin="round"
            className="text-tur-gray-500 pointer-events-none shrink-0 mr-2.5"
          >
