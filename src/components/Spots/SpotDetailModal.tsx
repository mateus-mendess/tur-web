...
export function SpotDetailModal({ spot, isOpen, onClose }: SpotDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      {/* Darkened Blur Backdrop showing top part of page */}
      <div
        className="absolute inset-0 bg-black/65 backdrop-blur-sm transition-opacity"
        onClick={onClose}
      />

      {/* Sheet Container: Full width, bottom-0, showing top gap of underlying page */}
      <div
        className="relative w-full h-[calc(100vh-3.5rem)] md:h-[calc(100vh-4rem)] mt-14 md:mt-16 bg-tur-bg text-tur-dark overflow-y-auto flex flex-col font-inter border-t border-tur-dark animate-in slide-in-from-bottom duration-300 rounded-none z-10"
        role="dialog"
        aria-modal="true"
      >

        {/* 1. TOP BAR (Header Superior Fixo) */}
        <header className="sticky top-0 z-40 w-full bg-tur-bg/95 backdrop-blur-md border-b border-tur-dark/15 px-6 md:px-12 py-4 flex items-center justify-between">
          {/* Left: Spot Title & Author Info */}
          <div className="flex items-center gap-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2">
              <span className="font-dm-sans font-bold text-base text-tur-dark uppercase tracking-wider">{spot.name}</span>
              <span className="font-inter text-xs text-tur-gray-700">por <strong className="text-tur-dark font-semibold">{spot.author.name}</strong></span>
            </div>
          </div>

          {/* Right: Actions (Save, Share, and Close Button) */}
        ...
        {/* Floating Score Badge - menor tamanho */}
        <div className="absolute left-6 md:left-12 top-[88px] border border-tur-dark p-2 w-16 sm:w-20 text-center flex flex-col items-center justify-center rounded-none bg-tur-bg z-10 pointer-events-none">
          <span className="font-inter text-[8px] sm:text-[9px] font-extrabold uppercase tracking-wider text-tur-accent">NOTA</span>
          <span className="font-dm-sans text-xl sm:text-2xl font-extrabold text-tur-dark leading-none my-0.5">
            {spot.rating || '4.8'}
          </span>
          <span className="font-inter text-[8px] sm:text-[9px] text-tur-dark/60 font-semibold">/ 5.0</span>
        </div>
        ...
        <main className="flex-1 w-full px-6 md:px-12 pt-8 pb-16 space-y-12">
          ...
          <section className="relative pt-4 pb-2">
            ...
            {/* Centered Published Date */}
            <div className="text-center mb-6">
              <span className="font-inter text-xs font-extrabold uppercase tracking-[2px] text-tur-dark/60">
                PUBLICADO - {spot.publishedAt ? spot.publishedAt.replace(/\bDE\b/gi, ',').replace(/\s+/g, ' ').toUpperCase() : 'OUTUBRO, 2023'}
              </span>
            </div>
          ...
          {/* B. MEDIA GALLERY (Asymmetric 4-Image Grid Layout aligned with Content) */}
          <section className="w-full">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 w-full">

              {/* Left Column: Large Featured Vertical/Main Image */}
              ...
              {/* Right Column: 1 Horizontal Top Image + 2 Square Bottom Images */}
          ...
          {/* C. INFO LAYOUT (2 MAIN COLUMNS: LEFT 7/8-COLS, RIGHT 5/4-COLS) */}
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch pt-6">

            {/* LEFT COLUMN: SOBRE (TOP) + ACESSIBILIDADES & CATEGORIAS (BOTTOM 2 SUB-COLUMNS WITH VERTICAL BORDER EXTENDING TO BOTTOM) */}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 font-inter text-base sm:text-lg text-tur-dark/90 leading-relaxed font-normal">
                  {spot.description ? (
                    ...
                    <p>O local oferece uma experiência visual memorável e contato direto com a história e cultura da região.</p>
                  )}
                </div>
              </div>

              {/* Quadrado Abaixo do Sobre: Linha vertical divisória estendida até a base da linha da localização */}
              <div className="pt-6 border-t border-tur-dark/15 grid grid-cols-1 sm:grid-cols-2 gap-8 items-stretch flex-1">

                {/* Sub-quadrado 1: Acessibilidade (Linha vertical estendida até o fim da coluna) */}
                  ...
                  {spot.accessibility && spot.accessibility.length > 0 ? (
                    <ul className="space-y-2.5">
                      {spot.accessibility.map((item, idx) => (
                        <li key={idx} className="flex items-center gap-2.5 font-inter text-sm sm:text-base font-medium text-tur-dark">
                          <span className="text-tur-accent font-black text-base sm:text-lg">+</span> {item}
                        </li>
                      ))}
                    </ul>
                  ) : (
                ...
                {/* Sub-quadrado 2: Categorias */}
                <div className="space-y-4 sm:pl-2 h-full">
                  <h4 className="font-dm-sans text-lg font-bold uppercase text-tur-dark">
                    Categorias
                  </h4>
                  <ul className="space-y-2.5">
                    {(spot.tags || [spot.category ? spot.category : 'Turismo', 'Ponto Turístico', 'Vistas']).map((tag, idx) => (
                      <li key={idx} className="flex items-center gap-2.5 font-inter text-sm sm:text-base font-medium text-tur-dark">
                        <span className="text-tur-accent font-black text-base sm:text-lg">•</span> {tag.replace(/^#/, '')}
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {/* RIGHT COLUMN: LOCALIZAÇÃO (TOP) + MAPA + BOTÃO GOOGLE MAPS (ABAIXO DO MAPA) */}
            <div className="lg:col-span-5 xl:col-span-4 space-y-6 flex flex-col justify-between h-full">

              {/* Localização (Sem ponto de referência) */}
              <div className="space-y-2">
                <div className="flex items-start gap-3">
                  <MapPinIcon className="w-5 h-5 text-tur-accent shrink-0 mt-0.5" />
                  <div>
                    <h4 className="font-dm-sans font-bold text-base text-tur-dark uppercase">Localização</h4>
              ...
              {/* Botão "Abrir no Google Maps" posicionado ABAIXO do mapa */}
          ...
          {spot.reviews && spot.reviews.length > 0 && (
            <section className="space-y-6 pt-8 border-t border-tur-dark/15">
              ...
              <div className="mt-8 sm:mt-10 border-y border-dashed border-tur-dark divide-y divide-dashed divide-tur-dark">
                {spot.reviews.slice(0, visibleReviewsCount).map((review) => (
                  ...
                  >
                    ...
                    {/* Direita: Texto da Avaliação (Centralizado verticalmente e Justificado) */}
              ...
              {/* Botão "..." para carregar mais 5 comentários */}
